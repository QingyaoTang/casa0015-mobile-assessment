...
class _RegisterPageState extends State<RegisterPager> {
  ...
  Future<void> registerRequest(BuildContext context) async {
    ...
    if(username.isEmpty || password.isEmpty || rePassword.isEmpty){
      showErrorToast("username or password not null");
      return;
    }

    if(password != rePassword){
      showErrorToast("password no the same");
      return;
    }
    ...
    if (res.isOk()) {
      ...
      if (res.statusCode == 200) {
        // debugPrint("注册成功");
        showCorrectToast("register success");
        Navigator.pop(context);
      } else {
        showErrorToast(res.statusMessage);
        debugPrint("request error，fail");
      }
    } else {
      //请求失败
      showErrorToast(res.statusMessage);
      debugPrint(
           "request error ${res.statusCode}  ${res.statusMessage}");
    }
  }
}
